...
md"""
### Reuse: `AcceleratedKernels.jl`

Just like last week, where after hand-writing a handful of parallel reductions we could simply reach for `OhMyThreads.treduce`, we do **not** have to write the `@localmem` reduction by hand on the GPU either.

[`AcceleratedKernels.jl`](https://github.com/JuliaGPU/AcceleratedKernels.jl) provides portable, backend-agnostic high-level primitives built directly on top of `KernelAbstractions`. They mirror the `OhMyThreads.jl` vocabulary from last week, but run on **any** backend -- the CPU as well as every GPU vendor:

| `OhMyThreads.jl` (CPU) | `AcceleratedKernels.jl` (portable) |
| --- | --- |
| `tforeach`   | `foreachindex` |
| `tmap`       | `map` |
| `treduce`    | `reduce` |
| `tmapreduce` | `mapreduce` |

So our hand-written `naive_shmem_mapsum` / `my_mapsum` is really just a special case of `AK.mapreduce` -- and the library takes care of the local-memory reduction, the atomics, and the work-group sizing for us.
"""
...
# ╔═╡ adfe4923-3365-4292-a153-ac1e3688155f
md"""
#### Compute follows the **backend**, not the data

The high-level array operations from `GPUArrays` are *implicitly* parallel **only because they dispatch on the array type**: `mapreduce(f, +, ::CuArray)` runs on the GPU because the data is a `CuArray`.

But the very same operation over a lazy host iterator like `1:N` is just an ordinary, sequential CPU reduction -- there is no array to dispatch on, so nothing moves to the device:
"""
...
md"""
`AcceleratedKernels.jl` decouples *where the computation runs* from *where the data lives*: the backend is just an argument. A `1:N` range is `isbits`, so it is captured straight into the kernel and indexed **on the device** -- no `CuArray`, no explicit data movement required.
...
f′(A::MyMatrix) = λ ->      my_mapsum((v) -> v^2 / (v - λ)^2, A.v)
...
manifest_format = "2.0"
project_hash = "cf08d652a3cc7aa654219b6c8ed26c29e57f447f"
...
git-tree-sha1 = "5abda002700cd788355a82080cc47927185415f7"
...
deps = ["Artifacts", "JLLWrappers", "Libdl", "TOML", "Zlib_jll", "Zstd_jll", "libLLVM_jll"]
git-tree-sha1 = "af4c4e3c93bfb964c643ae6996fb0c188c3e7aca"
...
git-tree-sha1 = "61b917828b9d9bf6da81876dc3798f774b419707"
...
git-tree-sha1 = "46515d88b6e994bb237442c3edda7f5f3a98a82f"
...
[[deps.pocl_jll]]
deps = ["Artifacts", "Clang_unified_jll", "Hwloc_jll", "JLLWrappers", "LLD_unified_jll", "Libdl", "OpenCL_Headers_jll", "OpenCL_jll", "SPIRV_Tools_jll", "Zstd_jll"]
git-tree-sha1 = "0c19849abf23d6f62f1bcbbb7942d6d9842c87c8"
