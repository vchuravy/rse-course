...
# ╔═╡ 9a9ebdf3-75a0-4315-a698-2c8b3d2ac3fb
...
# ╔═╡ 2ca60a94-9ebc-4e0d-9f16-19eb919a4b49
function profile_map(map, n, k=8)
    for i = 1:n
        Ms = [rand(100,100) for i in 1:(k * Threads.nthreads())]
		map(svdvals, Ms)
    end
end

# ╔═╡ bfc746a9-6238-4b55-9545-6c3dff0193e0
@profview profile_map(map, 1);

# ╔═╡ 3bcc6045-9cbe-4ecc-8741-19fdfe42df5a
@profview profile_map(tmap, 1);

# ╔═╡ 9cae6c67-fb58-4125-bdcc-9374f66fa20b
@profview profile_map(map, 10)

# ╔═╡ da663d9c-7e00-4a9b-b6e2-10198a299c2b
@profview profile_map(tmap, 10)

# ╔═╡ d1108e11-5be1-410d-a0f3-a3b6f20982ef
md"""
!!! note
    By default most Julia profilers only show time spent in Julia.

    ```julia
	@profview profile_map(map, 10) C=true
	```
    Will also show time spent in the runtime and in C.
"""

# ╔═╡ 10a43b34-2336-424a-815f-65ae04723a33
begin
	blas_edge
	@profview profile_map(map, 10) C = true
end
...
begin
	blas_edge
	@profview profile_map(tmap, 10) C = true
end

# ╔═╡ b1c655d2-4b59-428d-8cae-18e758445bc0
md"""
!!! note
    The Julia profiler will only show work done on Julia threads.
    Libraries like BLAS uses additional threads, and the use of a system profiler is needed.
"""

# ╔═╡ 314525f9-3464-4d46-bf4f-a826783175ac
question_box(
md"""
When `BLAS.set_num_threads(Sys.CPU_THREADS)` we see a function called `exec_blas_async_wait` dominating the time spent in BLAS.
...
manifest_format = "2.0"
project_hash = "ce08a1c165ee7821a054f00037f02370c9847def"
